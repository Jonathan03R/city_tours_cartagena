...
/// - Validación profesional: autovalidación, mensajes claros y formateadores de entrada.
/// - UX mejorada: secciones claras, feedback animado, focus traversal y deshabilitado seguro.
/// - Mejor manejo de precios: fuente agencia > global, validaciones y mensajes de error consistentes.
...
class _AddReservaFormState extends State<AddReservaForm> {
  ...
  void initState() {
    ...
    // Inicializa agencia si viene prefijada
  ...
  double _parseMoneda(String input) {
    ...
    // Si contiene coma y punto, asume coma como decimal si aparece al final
    // Ej.: "1.234,56" -> remove dots, replace comma with dot.
  ...
  double _precioAsiento({
    ...
    if (agencia != null) {
      final precioAg = turno == TurnoType.manana ? agencia.precioPorAsientoTurnoManana : agencia.precioPorAsientoTurnoTarde;
      if (precioAg != null && precioAg > 0) return precioAg;
    }
    if (config != null) {
      return turno == TurnoType.manana ? config.precioGeneralAsientoTemprano : config.precioGeneralAsientoTarde;
    }
    return 0.0;
  }

  double get _costoAsientoActual {
    return _precioAsiento(turno: _selectedTurno, config: _config, agencia: _selectedAgencia);
  }
  ...
  void _onValuesChanged() {
    if (!mounted) return;
    // No setState aquí solo por recomputación; UI usa getters derivados.
    setState(() {});
  }
  ...
  Future<void> _showSnackDialog(String msg, {bool isError = false}) async {
    await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(msg, style: TextStyle(color: isError ? Colors.red.shade300 : Colors.green.shade800)),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) {
    // Observa la config para actualizar precio si cambia
                          ...
                          children: [
                            ...
                            _LabeledField(
                              ...
                              child: TextFormField(
                                ...
                                textInputAction: TextInputAction.next,
                                onFieldSubmitted: (_) => _focusPax.requestFocus(),
                            ...
                            _LabeledField(
                              ...
                              child: TextFormField(
                                ...
                                minLines: 2,
                                maxLines: 4,
                                decoration: const InputDecoration(hintText: 'Detalles adicionales (opcional)'),
                              ),
                            ),
                          ],
                        ),
                        ...
                        child: Column(
                          ...
                          children: [
                            _InfoChip(
                              ...
                              warning: _costoAsientoActual <= 0 ? 'Revisa la configuración de precios' : null,
                            ),
                            const SizedBox(height: 8),
                            AnimatedSwitcher(
                              ...
                              child: _EstadoBox(
                                key: ValueKey('${_estadoActual.name}-${_costoTotal.toStringAsFixed(2)}-${_saldo.toStringAsFixed(2)}'),
  ...
  Widget _buildAgencia(ColorScheme cs) {
    if (widget.agenciaId != null) {
      return _isLoadingAgencia
          ? _LoadingLine(text: 'Cargando agencia…')
          : Row(
              children: [
  ...
  String _currency(num n) {
    // Usa configuración local actual
...
class _TotalesBox extends StatelessWidget {
  const _TotalesBox({super.key, required this.total, required this.diferencia, required this.formatter});
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            ...
            const SizedBox(width: 8),
            Text('Costo total: ', style: const TextStyle(fontWeight: FontWeight.w600)),
